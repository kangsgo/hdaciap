...
Phase 3  re2.csv          ──►  re2_n.csv        (activity_qualifier == '=', CID-sorted)
Phase 4  re2_n.csv        ──►  re3.csv          (AID-grouped relative fold & log)
Phase 5  re3.csv          ──►  re4.csv          (pairwise CID median cycle)
Phase 6  re4.csv          ──►  re5.csv          (CID deduplication, keep first)
Phase 7  re3.csv + re5.csv ──►  re5.csv         (pIC₅₀ via relative_value & first_mean)

Phase 3–7 (qualifier pipeline) build on the IC50-cleaned output of Phase 2.
"""
...
# =============================================================================
# Phase 3 — Qualifier filter + CID-frequency sort  (re2.csv → re2_n.csv)
# =============================================================================


def process_re2_qualifier(input_file: str) -> None:
    """Filter re2.csv for exact activity values and sort by CID frequency → re2_n.csv.
...
# =============================================================================
# Phase 6 — CID deduplication  (re4.csv → re5.csv)
# =============================================================================


def process_re4_to_re5(input_file: str) -> None:
    """Deduplicate re4.csv by CID, keeping the first occurrence → re5.csv.

    For rows with duplicate CIDs, only the row with the smallest positional
    index (i.e., first appearance) is retained.
    """
    ...
    if "CID" not in df.columns:
        print(f"[SKIP] {input_file}: missing CID column")
        return
    ...
    df.to_csv(output_file, sep="\t", index=False)
    print(f"[DONE] {input_file} -> {output_file} ({before} → {after} rows, {before - after} duplicates removed)")


# =============================================================================
# Phase 7 — pIC₅₀ computation  (re3.csv + re5.csv → re5.csv)
# =============================================================================


def compute_pic50(re3_path: str, re5_path: str) -> None:
    """Enrich re5.csv with pIC₅₀ values using baseline from re3.csv.

    1. Read the first CID from ``re3.csv`` and compute the median of its
       ``activity_value`` → *first_mean*.
    2. Compute ``relative_value`` = ``relative_fold`` × *first_mean*.
    3. Compute ``relative_log`` = −log₁₀(relative_value × 10⁻⁶),
       i.e. pIC₅₀, rounded to 2 decimals.
    4. Write the enriched result back to ``re5.csv``.
    """
    # --- Read baseline from re3.csv ---
    try:
        re3 = pd.read_csv(re3_path, sep="\t", on_bad_lines="skip")
    except Exception as e:
        print(f"[SKIP] {re3_path}: failed to read — {e}")
        return

    if "CID" not in re3.columns or "activity_value" not in re3.columns:
        print(f"[SKIP] {re3_path}: missing CID or activity_value column")
        return
    ...
    print(f"[DONE] CID={first_cid}, first_mean(median)={first_mean:.4f}, rows={len(data)} → {re5_path}")
...
def main() -> None:
    ...
    Phases are run in order:
      ...
      3. re2.csv → re2_n.csv        (qualifier + CID-sort)
      4. re2_n.csv → re3.csv        (relative fold)
      5. re3.csv → re4.csv          (median cycle)
      6. re4.csv → re5.csv          (CID dedup)
      7. re3.csv + re5.csv → re5.csv (pIC₅₀)
    """
    root_dir = os.getcwd()
    ...
    # ---- Phase 3: re2.csv → re2_n.csv (qualifier pipeline) ----
    for current_dir, _, files in os.walk(root_dir):
        if "re2.csv" in files:
            input_path = os.path.join(current_dir, "re2.csv")
            process_re2_qualifier(input_path)
    ...
    # ---- Phase 6: re4.csv → re5.csv (CID dedup) ----
    for current_dir, _, files in os.walk(root_dir):
        if "re4.csv" in files:
            input_path = os.path.join(current_dir, "re4.csv")
            process_re4_to_re5(input_path)

    # ---- Phase 7: re3.csv + re5.csv → re5.csv (pIC₅₀) ----
    for current_dir, _, files in os.walk(root_dir):
        re3_path = os.path.join(current_dir, "re3.csv")
        re5_path = os.path.join(current_dir, "re5.csv")
        if os.path.isfile(re3_path) and os.path.isfile(re5_path):
            try:
                compute_pic50(re3_path, re5_path)
            except Exception as e:
                print(f"[ERROR] {current_dir}: {e}")
