...
# -*- coding: utf-8 -*-
"""
Clean all *molecules.csv files and produce classic1.csv, classic2.csv, and
classic.csv.
...
Phase 3 — Binary encoding (classic.csv)
---------------------------------------
1. Extract columns CID, name, smiles, activity from classic2.csv.
2. Rename activity → y and encode: Active → 1, Inactive → 0.
3. Write the result as classic.csv.
...
def build_classic(input_path: str, output_path: str) -> None:
    """
    Convert classic2.csv to binary-labelled classic.csv.

    Extracts columns CID, name, smiles, activity; renames activity to y and
    encodes Active → 1, Inactive → 0.
    """
    try:
    ...
    if missing_cols:
        print(f"  [SKIP] {os.path.basename(input_path)} missing columns: {missing_cols}")
        return
    ...
    print(f"    Binary labels: {len(out_df)} rows -> {os.path.basename(output_path)}")
...
def main() -> None:
    ...
    # Phase 3: binary encoding → classic.csv
    print("\n[INFO] Starting Phase 3: binary label encoding.\n")
    classic2_files = find_target_files(base_dir, pattern="classic2.csv")
    if not classic2_files:
        print("[INFO] No classic2.csv files found for encoding.")
        return

    for input_path in classic2_files:
        output_path = os.path.join(os.path.dirname(input_path), "classic.csv")
        print(f"[Phase 3] Processing: {input_path}")
        build_classic(input_path, output_path)
